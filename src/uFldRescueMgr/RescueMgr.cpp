...
//------------------------------------------------------------
// Procedure: postGameStatus()

void RescueMgr::postGameStatus()
{
  if(m_map_node_rescues.size() != 2)
    return;
  ...
  for(p=m_map_node_rescues.begin(); p!=m_map_node_rescues.end(); p++) {
    ...
    string team = vname;
    if(m_map_node_records[vname].getGroup() != "")
      team = m_map_node_records[vname].getGroup();
